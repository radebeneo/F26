 ...
 * src/db/sync-gw4.ts
 *
 * Gameweek 4 sync script. Performs three operations in sequence:
 *
 ...
 *
 *  2. For every user who does NOT have a GW4 squad, copy their GW3 squad
 *     (players + settings) into a new GW4 squad row.
 *
 ...
 *
 * Usage:
 *   node --env-file=.env.local --import tsx src/db/sync-gw4.ts
 */
...
// ─── Step 2: Copy GW3 → GW4 for users without a GW4 squad ──────────────────

const GW_SOURCE = 3; // copy FROM this gameweek
const GW_TARGET = 4; // copy TO this gameweek

async function copyMissingGw4Squads() {
  ...
  for (const user of allUsers) {
    ...
    const hasGw4 = squads.some((s) => s.gameweekId === GW_TARGET);
    ...
    // Find the GW3 squad to copy from; fall back to the highest existing GW
    const sourceSquad =
...
main().catch((err) => {
  console.error("❌ GW4 sync failed:", err);
  process.exit(1);
});
